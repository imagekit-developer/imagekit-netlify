...
export const onPreBuild = function() {
  ...
  export async function onBuild({
    ...
    const imagekitUrlEndpoint = process.env.IMAGEKIT_URL_ENDPOINT || urlEndpoint;
      ...
      return;
            ...
            imagesPath.forEach( mediaPath => {
              ...
              try {
                ...
                netlifyConfig.redirects.unshift({
                  from: `${mediaPath}/*`,
                  to: getRedirectUrl({imagekitUrlEndpoint ,imagekitFakeAssetPath,transformations,remoteHost:host as string}),
                  status: 302,
                  force: true,
                });
              } catch (error) {
                console.log("Error during rewrite", error)
                globalErrors.push({page:mediaPath,
